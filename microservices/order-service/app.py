...
def ready():
    try:
        ...
        return jsonify({"status": "not-ready", "service": "order-service"}), 503
...
def create_order():
    try:
        ...
        cart_response = requests.get(
            f"{CART_SERVICE_URL}/api/cart/{user_id}", timeout=5
        )
        ...
        for item in cart["items"]:
            try:
                product_response = requests.get(
                    f'{PRODUCT_SERVICE_URL}/api/products/{item["productId"]}', timeout=5
                )
                if product_response.status_code == 200:
                    ...
                    return (
                        jsonify(
                            {"error": f'Product {item["productId"]} not available'}
                        ),
                        400,
                    )
            except requests.RequestException as e:
                return (
                    jsonify({"error": f"Failed to fetch product details: {str(e)}"}),
                    500,
                )
